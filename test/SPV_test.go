...
func GenerateTransaction(outCash int, inAccount, toAccount, prevTxID string, outIdx int) *transaction.Transaction {
	var (
	...
	tx := transaction.Transaction{[]byte("This is the Base Transaction!"),
		[]transaction.TxInput{txIn}, []transaction.TxOutput{txOut}}
	tx.SetID()
	return &tx
}
